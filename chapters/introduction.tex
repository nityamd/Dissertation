...
Since we are limited in our observations by the fact that we cannot observe galaxies in a time-resolved fashion (the time scales are too long), theories regarding galaxy formation rely on computational simulations that evolve the initial conditions to the present day. We can compare the galaxies at each epoch in a simulation to the populations observed at different redshift, assuming large-scale homogeneity. As \citet{somerville15a} describe, these simulations incorporate the physics of $\Lambda$CDM, magnetohydrodynamics, and gravitation. Additionally, these models need to account for a variety of processes on subresolution scales that involve star formation, stellar and AGN feedback, and gas cooling to be able to correctly predict the observed Universe.\\

\begin{figure}
\includegraphics[width=\textwidth]{figures/hubble.jpeg}
\caption[ The Hubble Sequence: A classification scheme for galaxy morphologies (SDSS color images): Originally proposed by Hubble, this image captures the diversity of galaxy morphologies in the Universe. The broad categories are elliptical and spiral, with a whole range of intermediate morphologies. A variation of the spiral galaxies are barred spirals that have an arm running through them. This ``tuning fork" diagram is still of relevance to astronomers as there are some known correlations of galaxy properties with morphology.]
{The Hubble Sequence: A classification scheme for galaxy morphologies (SDSS color images): Originally proposed by Hubble, this image captures the diversity of galaxy morphologies in the Universe. The broad categories are elliptical and spiral, with a whole range of intermediate morphologies. A variation of the spiral galaxies are barred spirals that have an arm running through them. This ``tuning fork" diagram is still of relevance to astronomers as there are some known correlations of galaxy properties with morphology.
\label{fig:hubble_classification}}
\end{figure}

There are still many questions to be answered including and not limited to: how and where the baryons collapse, how galactic disks stabilize, the drivers and mechanisms of galactic feedback and what ultimately causes the relative frequency of different galaxy types in the observable Universe. There are thus many holes in this narrative of galaxy formation and evolution and astronomers are working to bridge this gap in two main ways: from the observational side, this involves accurate measurements of galaxy properties across cosmic time and from the (theoretical) side of computational astrophysics, this involves fine-tuning the sub-grid models of galaxy formation and evolution simulations so as to be able to reproduce the estimated from the properties. These both take from and in turn inform the parameters in the $\Lambda$CDM model of the Universe that cosmologists are engaged in refining.\\
...
quantified as ``luminosity,'' the amount of energy emitted by a luminous object
...
(10--100 $\mu$m) is a tracer of the star formation rate as well. I
...
scattering continues until the photon is absorbed on dust, or a rare
...
The bluer Balmer emission lines  such as H$\beta$ are intrinsically weaker, and often substantially
dust-reddened. Collisionally excited lines such as the [OII] doublet at 3727
...
The galaxy spectrum contains an imprint of its Star Formation History (SFH), stellar metallicity and abundance pattern, stellar Initial Mass Function (IMF), total stellar mass and the nature of its gas and dust content. However, to be able to interpret the details in a spectrum accurately, one has to rely on evolutionary synthesis models or what are called Stellar Population Synthesis (SPS) models. Typically these models contain a comprehensive library of stellar spectra of stars across ages, masses and luminosities. The earliest such models (\citealt{1971ApJS...22..445S, faber_1972}) relied on using a linear combination of the diverse individual stellar spectra to reconstruct the integrated light emission from a galaxy. However, populations of stars do not come in arbitrary mixes --- stars are formed together in groups that evolve over time in a regular way. More recent models (\citealt{1995ApJS...96....9L, schulz_2002, bruzual_stellar_2003}) involve setting handful of global parameters such as the IMF, the SFR and the rate of chemical enrichment, and using the laws of stellar evolution and of stellar atmospheres to predict the emergent integrated spectrum of the population as it evolves in time.\\

Broadly speaking, SPS models require the following ingredients \citep{walcher_2011}: an IMF, stellar evolutionary tracks (isochrones), stellar spectral libraries, and a SFR and chemical evolution model. The choice of IMF (\citealt{1955ApJ...121..161S, 2001MNRAS.322..231K, 2003PASP..115..763C}) determines the distribution of stellar masses initially and the spectral libraries convert the stellar evolution outputs such as surface gravity and effective temperature into observable Spectral Energy Distributions (SEDs). The spectral libraries can either be calculated theoretically (\citealt{coelho_2005, martins05a}) or on a purely empirical basis and are oftentimes a combination of both. Some notable modern stellar spectral libraries constructed from observations are ELODIE \citep{2004astro.ph..9214P}, STELIB \citep{leborgne_2003}, MILES \citep{2006MNRAS.371..703S}, and most recently MaStar \citep{yan19a}.
...
Photometry provides useful constraints on stellar mass.  A useful
...
4000$ index (which is representative of the break at $4000$ \AA, found in
...
agreement (e.g. \citealt{salim_uv_2007}) or using non-parametric SFH's.
...
, the Sloan Digital Sky Survey (SDSS; \citealt{york00a,
...
in New Mexico, United States \citep{york00a,
...
structure and dynamics or its global spectrum. To address this
...
redshifts of $0 < z < 4$, there is a tight relationship between galaxy
...
% It has also been suggested that the scatter increases as a function
% of stellar mass (Ilbert 2015), thus suggesting that more massive
% galaxies might have a more diverse set of SFHs that have not been
% yet fully explored.\\
...
external processes such as environmental effects that strip the
galaxy of its gas content \citep{peng_mass_2012,
  2012ApJ...757...85G}. As \citet{blanton_physical_2009-1} point out,
...
simulations are calibrated are in error in a systematic way, that error
