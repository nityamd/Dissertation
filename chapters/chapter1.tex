...
Determining the formation history of galaxies is one of the most important tasks in astronomy. The average star formation rates of galaxies have reduced with cosmic time (as reviewed by \citealt{madau_cosmic_2014}) and star-forming galaxies ``transition" to red and quiescent ones over that time \citep[\emph{e.g.}][among others]{peng_mass_2010, ilbert_mass_2013, muzzin_evolution_2013, moustakas_primus_2013, tomczak_galaxy_2014}. Many mechanisms have been proposed to explain the observed patterns of galaxy evolution but there are many unresolved questions regarding them. These mechanisms are known to have some correlation with the environment, due to the fact that galaxy type depends on environment in the present day \citep{dressler_galaxy_1980, blanton_physical_2009-1}.\\
...
\label{ch1_data}
The sample on which we perform our measurements is based on the NASA-Sloan Atlas (NSA), a nearby galaxy sample which includes optical and ultraviolet imaging from SDSS and GALEX. We use NSA version {\tt v0\_1\_2}, whose redshift range extends up to $z = 0.055$ and includes SDSS (Data Release $9$) and GALEX imaging for galaxies. The total number of objects in this NSA catalog is ${\sim}150,000$. We define two samples from the NSA, one that we use to define environment (Environment Determining Population or EDP, hereafter) and one for which we determine star formation rates (Star Formation Rate Population or SFRP hereafter). We note that the SFRP is a subsample of the EDP. To get our EDP, we impose a volume-limited cut on this sample by retaining only galaxies with $-24.5 <M_{r}< -18.5$, which leaves us with $95,638$ galaxies. The imaging data from which we infer the star formation rates thus span 11 bandpasses: \emph{F} and \emph{N}, the Far-UV and  Near-UV bands from GALEX; \emph{u, g, r, i} and \emph{z}, the optical bands from SDSS; and \emph{W1, W2, W3} and \emph{W4}, the IR imaging bandpasses from WISE. Together, these cover imaging information of every galaxy from ~ 1000 \AA - 22000 \AA.\\
The star formation rate determinations we use here require both UV data 
and IR data. About 80\% of our galaxies have UV data from GALEX 
analyzed in the NSA. To obtain infrared imaging for the data we use 
the Wide-Field Infrared Survey Explorer (WISE) data and match the 
objects from the NSA with the ALLWISE Source Catalog to get the 
four-band infrared fluxes for each galaxy. About 99\% of galaxies in 
...
and $W4$ bands). While the \emph{k}-corrected magnitudes are 
available for the optical and UV bands, we had to reasonably account for them
for the WISE bands. To this end, the \emph{k}-corrections were obtained for
the WISE bands iteratively by using MAGPHYS to re-calculate them and were
found to converge after the second iteration.\\

\begin{figure}
\includegraphics[width=\textwidth]{figures/number_of_galaxies}
\caption[Data: The local sample distribution binned along optical 
and IR colors. The optical color \emph{N-r} is a logarithm of the ratio
of the fluxes from the \emph{r} band to the \emph{N} band and is
representative of how red the galaxy is (i.e., higher the \emph{N-r},
the redder the galaxy). The IR color \emph{[W1]-[W3]}, similarly, is
the logarithm of the ratio of the flux from the IR bands, \emph{[W3]}
and \emph{[W1]}.]{Data: The local sample distribution binned along optical 
and IR colors. The optical color \emph{N-r} is a logarithm of the ratio
of the fluxes from the \emph{r} band to the \emph{N} band and is
representative of how red the galaxy is (i.e., higher the \emph{N-r},
the redder the galaxy). The IR color \emph{[W1]-[W3]}, similarly, is
...
\caption[The SSFR measurements and environment 
    ...
    bins; all bins with fewer than $5$ galaxies were discarded in this 
...
panel of Fig \ref{fig:panel_plot} along with the number distribution of 
...
left panel of Fig. \ref{fig:panel_plot}, to assign an SSFR to each 
individual galaxy.\\
...
The lower right panel of Fig. \ref{fig:panel_plot} shows the mean 
...
\label{ch1_env}
We have identified above a population of galaxies isolated in 
color space that appear to have dust-extincted star formation, 
...
of sight velocity window of $v_{los} = \pm 1000 {\rm km}/{\rm s}$. For each
galaxy for which we have star formation measurements, we count its 
number of neighbors ($n_{\rm env}$) in this cylinder (Fig. \ref{fig:panel_plot}) in
the Environment Defining Population.\\

\begin{figure}
\includegraphics[width=\textwidth]{figures/2_env_plot.pdf}
\caption[The Ultraviolet and MAGPHYS SSFR's plotted against each other as a function of the environment; We notice two distinct set of outliers that seem to have lower UV SSFR's but similar environments to the galaxy bins with the same MAGPHYS SSFR's]{The two star formation rate estimates (from Fig. \ref{fig:panel_plot}) plotted against each other as a function of the environment; We notice two distinct set of outliers that seem to have lower UV SSFR's but similar environments to the galaxy bins with the same MAGPHYS SSFR's.
...
To identify the edges, we use \texttt{Mangle} \citep{swanson_mangle}, 
...
The mean environments, quantified by $(\langle n_{\rm env}\rangle + 1)$
in each bin are shown in the upper right panel of Fig. \ref{fig:panel_plot} 
...
\label{ch1_results}
Fig. \ref{fig:env_uv_magphys} shows the relationship between the two SSFR measurements,
with points colored according to the mean environment, for each bin 
in color-color space from Fig. \ref{fig:panel_plot}. There is a set of bins in 
...
Fig. \ref{fig:env_uv_magphys}. 

We identify these outliers more specifically in Fig. \ref{fig:outliers}:\emph{(a)}, as 
...
values (left of the square) are identified as ``UV BOX" in Fig. \ref{fig:outliers}:\emph{(a)}. \\
...
When we return to the color-color space (Fig. \ref{fig:outliers}:\emph{(b)}) and 
...
of environments in these three regions, the result of which is plotted in Fig. \ref{fig:munny_plot}.
...
{Probability density functions of the Environments of the three populations described in Fig \ref{fig:outliers}
...
From the analysis presented above, we can conclude the following:
 \begin{itemize}
 \item{From Fig \ref{fig:panel_plot}, we see that SSFR\textsubscript{MAGPHYS} identifies a region in the color-color space as dust-obscured star forming galaxies and correlates better with the environments of the galaxies.}
 ...
 \item{Comparing the environment distribution of the Outliers relative to the galaxies with (a) the same SSFR\textsubscript{MAGPHYS}'s as the Outliers and (b) the same SSFR\textsubscript{UV}'s as the Outliers (Fig. \ref{fig:munny_plot}), we find that the Outliers indeed have a similar environment distribution to the galaxies that have the same SSFR\textsubscript{MAGPHYS}'s, i.e., they seem to favor lower environment densities mimicking the behavior of star-formers.}
 \end{itemize}
 
The above results are consistent with the following
scenario, which we consider most likely. 
First, as others have concluded, UV star formation 
rates, because of dust extinction, can underestimate the 
star formation, whereas star formation rates that account 
for IR re-emission by dust account for star formation
more completely. Second, the fact that the outlier galaxies
(with relatively high MAGPHYS star formation
but low UV star formation)
have environments similar to the population with high 
UV and MAGPHYS star formation
reflects the fact that the MAGPHYS estimates are 
more accurate and that environment traces the total
star formation rate. Thus, our result provides a new 
piece of evidence, independent of how well the galaxy 
SED is explained by the inferred star formation rate
and the SSP model, that accounting for IR emission 
can yield a more reliable star formation estimate.\\
 
An alternative scenario exists, which is that MAGPHYS
does not accurately estimate the star formation rates of
the outlier galaxies, but that the environment correlates
with dust content in such a way as to correct for the 
difference.

The fact that the UV star formation rates can be 
significant underestimates has consequences for the 
observations of low luminosity nearby galaxies which are 
too faint to have reliable IR information, but even 
more so for high redshift galaxies, for which rest-frame
IR observations can be difficult to obtain. The 
James Webb Space Telescope (JWST; \citealt{gardner06a}) will 
play a significant role in the next few years to provide
IR data for a larger sample of these faint galaxies.
Our results here reinforce the necessity of doing so
for a complete accounting of the star formation history
of the universe.
