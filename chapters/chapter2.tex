...
extended objects, in order to determine their internal
...
\section{The H$\delta_{\rm A}-{\rm D}_{\rm n}4000$ plane}
\label{indices}
...
\subsection{The ${\rm D}_{\rm n}4000$ break in Galaxy Spectra}
\label{d4000}
...
\citet{kauffmann_stellar_2003} use ${\rm D}_{\rm n}4000$, because
...
${\rm D}_{\rm n}4000$ indices for stellar populations. The evolutionary 
...
${\rm D}_{\rm n}4000$ is sensitive to the mean star formation rate over
...
and ${\rm D}_{\rm n}4000$ indices, the stellar mass-to-light ratios in the 
...
``model grid" in the H$\delta_{\rm A}$-${\rm D}_{\rm n}4000$ plane,
...
H$\delta_{\rm A}$-${\rm D}_{\rm n}4000$ plane thus obtained are 
plotted in Fig. \ref{fig:kauff_grid}.\\ 
...
These procedures assume that the fiber mass-to-light ratios and 
dust attenuation factors are applicable to the whole galaxy. These
assumptions could in principle be a source of systematic error,
...
bias by comparing galaxies with similar \emph{z}-band luminosities 
...
whose absolute magnitude in \emph{z}-band is between 
...
its spectra have very similar properties to those of 
...
differ in the extent of angular coverage, with the 
...
$${\rm D}_{\rm A}(z) = \frac{D_{\rm M}(z)}{1+z}, $$
where ${\rm D}_{\rm M}(z)$ is the co-moving distance at redshift $z$.
...
$$ {d}_{\rm new} = \frac{(1+z_{\rm new})\times {\rm D}_{\rm M}(z_{\rm obs}) \times d}{(1+z_{\rm obs}) \times  {\rm D}_{\rm M}(z_{\rm new})} $$
...
The dashed line corresponds to $x=y$. The colorbar represents 
...
to $x=y$. The colorbar represents the number of galaxies in each bin.
\label{fig:redshift_comparison}}
\end{figure}

I measure H$\delta_{\rm A}$ and ${\rm D}_{\rm n}4000$ indices in the standard
...
and side bands and the two ${\rm D}_{\rm n}4000$ bands, and store these five
...
measurements following the procedure in Section \ref{indices}.
...
${\rm D}_{\rm n}4000$ measures are. For each redshift $z$, I pick the galaxies 
...
using the grid in Fig. \ref{fig:kauff_grid}. ]{ \emph{Left:} The combined 
...
ratios using the grid in Fig. \ref{fig:kauff_grid}.
\label{fig:offset_quiver1}}
\end{figure}

\begin{figure}
\includegraphics[width=\textwidth]{figures/mlz_offset_b.pdf}
\caption[  \emph{Left:} The combined mean offset in $D_{\rm n}4000$-$h\delta_{\rm A}$ at $z=0.02$ from the full aperture measurements represented as a vector whose projections on the axes are the actual offsets in either direction. \emph{Right:} The offsets transformed to the z-band mass-to-light ratios using the grid in Fig. \ref{fig:kauff_grid}. ]{ \emph{Left:} The combined mean offset in $D_{\rm n}4000$-$h\delta_{\rm A}$ at $z=0.06$ from the full aperture measurements represented as a vector whose projections on the axes are the actual offsets in either direction. \emph{Right:} The offsets transformed to the z-band mass-to-light ratios using the grid in Fig. \ref{fig:kauff_grid}.
...
\caption[ \emph{Left:} The combined mean offset in $D_{\rm n}4000$-$h\delta_{\rm A}$ at $z=0.14$ from the full aperture measurements represented as a vector whose projections on the axes are the actual offsets in either direction. \emph{Right:} The offsets transformed to the z-band mass-to-light ratios using the grid in Fig. \ref{fig:kauff_grid}. ]{ \emph{Left:} The combined mean offset in $D_{\rm n}4000$-$h\delta_{\rm A}$ at $z=0.14$ from the full aperture measurements represented as a vector whose projections on the axes are the actual offsets in either direction. \emph{Right:} The offsets transformed to the z-band mass-to-light ratios using the grid in Fig. \ref{fig:kauff_grid}.
...
the 3$''$ and full aperture measurements. Fig. \ref{fig:offset_d4000} 
shows the mean offsets and standard deviations for the 
$D_{\rm n}4000$ measurements. Fig. 
...
shown in Fig. \ref{fig:kauff_grid}. Thus each offset in the 
...
offsets in each of the redshift bins binned along the ${\rm D}_{\rm n}4000$ 
...
Fig. \ref{dispersion_plot} summarizes these findings, showing
the mass-to-light ratio offset and standard deviation in each bin 
for each redshift.
We quantify these findings in Tables \ref{tab: mean_offset_table} 
...
${\rm D}_{\rm n}4000$-H$\delta_{\rm A}$ plane shown in 
Fig. \ref{fig:kauff_grid}.  Table \ref{tab: median_offsets} also 
...
small. Both Table \ref{tab: mean_offset_table} and Fig. 
...
to be less sudden than in reality.
...
and  \ref{tab: median_offsets} and Fig. \ref{dispersion_plot} yield 
